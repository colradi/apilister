...
var apilister = require("./apilist_prepareURL_voters.js");
 ...
 * @return  { "total": total, "totalVotes": totalVotes, "data": url_configs}
 */
async function getVotersTronairFormat(address){
    var voters = await getVoters(address);
    console.log(JSON.stringify(voters))
    var sum = 0;
    var tmp = voters.data.map(function (target){ sum+= target.votes; return [target.voterAddress, target.votes] });
    return { "size": voters.data.length, "balance": sum, "data": tmp};
}

module.exports.getVoters = getVoters;
module.exports.getVotersTronairFormat = getVotersTronairFormat;
...
getVotersTronairFormat("TMykaWQe9YinNb36yE5k4MH9VUEGyKgQQA").then(x => { checkVoters(x);  });



function checkVoters(info){
